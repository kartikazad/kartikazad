<div align="center">

# Hi, I'm Kartik Azad 👋

### Full Stack Developer · QA Automation 

Building practical web applications and automation frameworks with
**JavaScript, Next.js, React, Python, Selenium & Pytest.**

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/kartik-azad-02531b200/)
[![Email](https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:kartikazad421@gmail.com)

</div>

---

## 🚀 About Me

- 🎓 Computer Science Engineering graduate
- 💻 Full-stack development experience with **JavaScript, React, Next.js & databases**
- 🐍 Comfortable with **Python, C++, Java & SQL**
- 🛍️ Built and deployed **Mini Store**, a full-stack e-commerce application
- 🧪 Building **Selenium + Pytest automation frameworks** using the Page Object Model
- 🔄 Experience with **manual QA, smoke, functional, regression & negative testing**
- ⚙️ Learning and applying **GitHub Actions CI/CD** for automated test execution
- 🔍 Interested in building software that is both **functional and reliable**
- 📫 Reach me at **[kartikazad421@gmail.com](mailto:kartikazad421@gmail.com)**

---

## 🧰 Tech Stack

### 💻 Languages

![C++](https://img.shields.io/badge/C%2B%2B-%2300599C.svg?style=for-the-badge&logo=c%2B%2B&logoColor=white)
![Python](https://img.shields.io/badge/Python-3670A0?style=for-the-badge&logo=python&logoColor=ffdd54)
![Java](https://img.shields.io/badge/Java-%23ED8B00.svg?style=for-the-badge&logo=openjdk&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-%23323330.svg?style=for-the-badge&logo=javascript&logoColor=%23F7DF1E)
![SQL](https://img.shields.io/badge/SQL-%23336791.svg?style=for-the-badge&logo=postgresql&logoColor=white)
![HTML5](https://img.shields.io/badge/HTML5-%23E34F26.svg?style=for-the-badge&logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/CSS3-%231572B6.svg?style=for-the-badge&logo=css3&logoColor=white)

### ⚛️ Frameworks & Libraries

![React](https://img.shields.io/badge/React-%2320232a.svg?style=for-the-badge&logo=react&logoColor=%2361DAFB)
![Next JS](https://img.shields.io/badge/Next.js-black?style=for-the-badge&logo=next.js&logoColor=white)
![Node.js](https://img.shields.io/badge/Node.js-6DA55F?style=for-the-badge&logo=node.js&logoColor=white)

### 🧪 Testing & Automation

![Selenium](https://img.shields.io/badge/Selenium-%2343B02A.svg?style=for-the-badge&logo=selenium&logoColor=white)
![Pytest](https://img.shields.io/badge/Pytest-%230A9EDC.svg?style=for-the-badge&logo=pytest&logoColor=white)

- Page Object Model (POM)
- Explicit Waits
- Functional Testing
- Smoke Testing
- Regression Testing
- Negative Testing
- Automated UI Testing
- HTML Test Reporting
- Failure Screenshots

### 🗄️ Databases

![MongoDB](https://img.shields.io/badge/MongoDB-%234ea94b.svg?style=for-the-badge&logo=mongodb&logoColor=white)
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-%23316192.svg?style=for-the-badge&logo=postgresql&logoColor=white)

### 🛠️ Tools & Platforms

![Git](https://img.shields.io/badge/Git-%23F05033.svg?style=for-the-badge&logo=git&logoColor=white)
![GitHub](https://img.shields.io/badge/GitHub-%23121011.svg?style=for-the-badge&logo=github&logoColor=white)
![GitHub Actions](https://img.shields.io/badge/GitHub_Actions-%232088FF.svg?style=for-the-badge&logo=github-actions&logoColor=white)
![VS Code](https://img.shields.io/badge/VS%20Code-0078D7.svg?style=for-the-badge&logo=visual-studio-code&logoColor=white)
![Vercel](https://img.shields.io/badge/Vercel-%23000000.svg?style=for-the-badge&logo=vercel&logoColor=white)

---

## 📌 Featured Projects

### 🛍️ Mini Store

**Full-stack e-commerce application** built with Next.js, React and MongoDB.

- 🛒 Product catalog powered by MongoDB
- 🧺 Cart state management
- 🔌 REST API routes
- 📱 Responsive interface
- 🚀 Deployed with Vercel

**[🌐 Live Demo](https://mini-ecommerce-store-nine.vercel.app/)**

---

### 🧪 E-Commerce Selenium Automation Framework

**Python · Selenium WebDriver · Pytest · Page Object Model · GitHub Actions**

🟡 **IN PROGRESS**

A Selenium automation framework for testing the Mini Store e-commerce application.

- ✅ Homepage loading validation
- ✅ Cart state validation
- ✅ Add-to-cart automation
- ✅ Multiple-product scenarios
- ✅ Product-specific selection
- ✅ Product `Added ✓` state validation
- ✅ Explicit waits for reliable element interaction
- ✅ Page Object Model architecture
- ✅ Automatic failure screenshots
- ✅ Pytest HTML test reports
- ✅ GitHub Actions CI
- 📊 **Current test suite: 7/7 passing**

**[💻 View Project on GitHub](https://github.com/kartikazad/ecommerce-selenium-automation)**

> 🚧 The framework is actively being expanded with additional cart, edge-case, and end-to-end scenarios.

---

### 📋 Manual QA Test Suite

Manual QA testing project covering a B2B SaaS platform.

- 🔐 Authentication testing
- 📊 Dashboard testing
- 👥 User management
- 💳 Billing modules
- ✅ Smoke testing
- 🔄 Regression testing
- 🧩 Functional testing
- ⚠️ Negative testing

---

## 📊 GitHub Stats

<div align="center">

<img height="180" src="https://github-stats-extended.vercel.app/api?username=kartikazad&show_icons=true&theme=tokyonight&hide_border=true" />

<img height="180" src="https://github-stats-extended.vercel.app/api/top-langs/?username=kartikazad&layout=compact&theme=tokyonight&hide_border=true" />

</div>

---

## 🌐 Connect With Me

<div align="center">

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/kartik-azad-02531b200/)
[![Email](https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:kartikazad421@gmail.com)

<br><br>

![Profile Views](https://komarev.com/ghpvc/?username=kartikazad&color=blueviolet&style=flat)

</div>

---

<div align="center">

### 💡 "Build it. Test it. Improve it."

⭐ Thanks for visiting my profile!

</div>
